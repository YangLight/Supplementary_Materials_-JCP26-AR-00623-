...
# 创建输出文件夹
os.makedirs("pairplot_train", exist_ok=True)
...
with open('pairplot_train/energy_ace.dat', 'w') as fe, \
     open('pairplot_train/forces_ace.dat', 'w') as ff, \
     open('pairplot_train/stress_ace.dat', 'w') as fs:
    ...
    for ii in range(409):
        ats = read('train_mace.extxyz', str(ii))
        ats.calc = calc
        ...
        if (ii+1) % 20 == 0 or ii == 0:
            print(f"Processed {ii+1}/409 structures")

print("MACE 计算完成，开始提取参考数据...")
# 打开文件用于写入参考数据
with open('pairplot_train/energy_ref.dat', 'w') as fe, \
     open('pairplot_train/forces_ref.dat', 'w') as ff, \
     open('pairplot_train/stress_ref.dat', 'w') as fs:
    ...
    for ii in range(409):  # 假设共有 409 个结构
        ats = read('train_mace.extxyz', index=ii)  # 读取第 ii 个结构
...
print("读取原子数...")
atom_counts = []
for ii in range(409):  # 共409个结构
    ats = read('train_mace.extxyz', index=ii)
    atom_counts.append(len(ats))
atom_counts = np.array(atom_counts)

# 2. 处理能量数据 (转换为eV/atom)（保持不变）
print("处理能量数据...")
ref_energies = np.loadtxt('pairplot_train/energy_ref.dat', usecols=1)
calc_energies = np.loadtxt('pairplot_train/energy_ace.dat', usecols=1)

# 转换为eV/atom并格式化
with open('pairplot_train/energy_pairplot_train.dat', 'w') as f:
...
# 读取并格式化力数据
ref_forces = read_forces('pairplot_train/forces_ref.dat')
calc_forces = read_forces('pairplot_train/forces_ace.dat')

# 写入Excel可用格式（所有力排成一列）
with open('pairplot_train/forces_pairplot_train.dat', 'w') as f:
...
# 读取参考和计算的stress数据
ref_stress_flat = read_stress_simple('pairplot_train/stress_ref.dat')
calc_stress_flat = read_stress_simple('pairplot_train/stress_ace.dat')
...
# 保存为stress_pairplot_train.dat - 改为与energy/force完全相同的格式
print("合并所有stress分量到单一文件...")
with open('pairplot_train/stress_pairplot_train.dat', 'w') as f:
...
print("\n所有数据处理完成！生成的文件：")
print("1. energy_pairplot_train.dat - 能量数据（参考值在前）")
print("2. forces_pairplot_train.dat - 所有力数据（展平，参考值在前）")
print("3. stress_pairplot_train.dat - 所有stress数据（9分量，展平，参考值在前）")
